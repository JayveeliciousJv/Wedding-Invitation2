...
export function Envelope({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <section className="section-dark relative min-h-screen flex flex-col items-center justify-center text-center px-4 sm:px-6 lg:px-8 overflow-hidden grain">
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse at center, color-mix(in oklab, var(--gold) 10%, transparent) 0%, transparent 60%)",
        }}
      />
      ...
      <div
        className="relative z-10 w-[320px] h-[200px] sm:w-[420px] sm:h-[260px] md:w-[520px] md:h-[320px]"
      ...
      >
        <AnimatePresence>
          {opened && (
            <motion.div
              ...
              className="absolute left-1/2 -translate-x-1/2 bottom-0 w-[88%] h-[120%] z-10 rounded-xl bg-warm-white border border-gold/30 shadow-soft flex flex-col items-center justify-center px-6"
              style={{ color: "var(--charcoal)" }}
            >
              <p className="font-script text-3xl md:text-4xl text-gold">You're Invited</p>
              <div className="w-12 h-px bg-gold/50 my-3" />
              <p className="font-serif text-lg md:text-2xl">
                {WEDDING.coupleOrder[0]} &amp; {WEDDING.coupleOrder[1]}
              </p>
              <p className="text-[10px] md:text-xs tracking-[0.3em] mt-2 uppercase opacity-70">
                {WEDDING.dateLabel}
              </p>
            </motion.div>
          )}
        ...
        <div
          className="absolute inset-0 z-20 rounded-md border border-white/10 shadow-soft pointer-events-none overflow-hidden"
          style={{
        ...
        >
          <div
            ...
            style={{
              background:
                "linear-gradient(to top right, transparent 49.7%, rgba(255,255,255,0.06) 49.85%, rgba(255,255,255,0.06) 50.15%, transparent 50.3%), linear-gradient(to top left, transparent 49.7%, rgba(255,255,255,0.06) 49.85%, rgba(255,255,255,0.06) 50.15%, transparent 50.3%)",
            }}
          />
          <div
        ...
        <motion.div
          ...
          transition={{ duration: 1.2, ease: [0.7, 0, 0.3, 1], delay: opened ? 0.3 : 0 }}
        >
          <div
            className="w-full h-full"
            style={{
              background:
                "linear-gradient(to bottom, oklch(0.50 0.001 60), oklch(0.42 0.001 60))",
      ...
      {!opened && (
        <motion.p
          ...
          className="relative z-10 mt-10 text-[10px] sm:text-xs tracking-[0.3em] text-muted-soft uppercase"
        >
